...
void publish_switch_discovery_payload(Param param)
{
  ...
  if (item != nullptr && item->variable != nullptr)
  {
    ...
    String value = (item->variable != nullptr && strlen((const char *)item->variable) > 0) ? String((const char *)item->variable) : "0";
    syslog_ng("publish_switch_discovery_payload  state " + String(switch_name) + " state_topic " + String(state_topic) + " (const char *)item->variable " + value);
...
void publishVariablesListToMQTT()
{
  ...
  int arraySize = sizeof(preferencesArray) / sizeof(preferencesArray[0]); // Determine the number of elements in the array
...
void onMqttMessageHA(char *topic, char *payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index, size_t total)
{
  ...
  if (strcmp(topic, (mqttPrefix + String("set/preferences/all")).c_str()) == 0)
  {
    ...
    for (int i = 0; i < sizeof(preferencesArray) / sizeof(PreferenceItem); ++i)
    {
      ...
      if (jsonDoc.containsKey(preferencesArray[i].key))
      {
        ...
        {
        case DataType::FLOAT:
          if (preferencesArray[i].preferences->getFloat(preferencesArray[i].key) != jsonDoc[preferencesArray[i].key].as<float>())
          {
            *(float *)preferencesArray[i].variable = jsonDoc[preferencesArray[i].key].as<float>();
            if (preferencesArray[i].preferences->putFloat(preferencesArray[i].key, jsonDoc[preferencesArray[i].key].as<float>()) == 0)
            {
          ...
          break;
        case DataType::STRING:
          if (preferencesArray[i].preferences->getString(preferencesArray[i].key) != jsonDoc[preferencesArray[i].key].as<const char *>())
          {
            *(String *)preferencesArray[i].variable = jsonDoc[preferencesArray[i].key].as<const char *>();
            if (preferencesArray[i].preferences->putString(preferencesArray[i].key, jsonDoc[preferencesArray[i].key].as<const char *>()) == 0)
            {
              syslog_ng("mqtt error save pref " + String(preferencesArray[i].key));
            }
          }
          break;
        case DataType::BOOLEAN:
          if (preferencesArray[i].preferences->getBool(preferencesArray[i].key) != jsonDoc[preferencesArray[i].key].as<bool>())
          {
            *(bool *)preferencesArray[i].variable = jsonDoc[preferencesArray[i].key].as<bool>();
            if (preferencesArray[i].preferences->putBool(preferencesArray[i].key, jsonDoc[preferencesArray[i].key].as<bool>()) == 0)
            {
              syslog_ng("mqtt error save pref " + String(preferencesArray[i].key));
            }
          }
          break;
        case DataType::INTEGER:
          if (preferencesArray[i].preferences->getInt(preferencesArray[i].key) != jsonDoc[preferencesArray[i].key].as<int>())
          {
            *(int *)preferencesArray[i].variable = jsonDoc[preferencesArray[i].key].as<int>();
            if (preferencesArray[i].preferences->putInt(preferencesArray[i].key, jsonDoc[preferencesArray[i].key].as<int>()) == 0)
            {
              syslog_ng("mqtt error save pref " + String(preferencesArray[i].key));
            }
          }
