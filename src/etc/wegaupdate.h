...
void updatePage()
{
  server.send_P(200, PSTR("text/html"), UPDATE_page);
}
...
void errorPage()
{
  server.send_P(400, PSTR("text/html"), ERROR_page);
}
...
void update_f()
{
  if ((server_make_update || force_update) && UPDATE_URL && !making_update)
  {
    ...
    while (retryCount < maxRetries)
    {
      ...
      {
        syslog_ng("make_update: Failed to connect, retrying... (" + String(retryCount + 1) + "/" + String(maxRetries) + ")");
        retryCount++;
