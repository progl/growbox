...
void TaskTemplate(void *params)
{
    ...
    syslog_ng(String("TaskTemplate " + String(taskParams->taskName) + " received task ") + String(taskParams->taskName));
    ...
    for (int i = 0; i < taskCount; i++)
    {
        ...
        {
            counter = i;
            syslog_ng("TaskTemplate already found task! " + String(taskParams->taskName) + " TaskTemplateCounter " + String(counter));
            currentTask = &tasks[i];
            break;
        }
    }
    ...
    {
        syslog_ng("Adding task " + String(taskParams->taskName) + " " + String(taskParams->taskName) + " to tasks array.");
    ...
    {
        syslog_ng("TaskTemplate  " + String(taskParams->taskName) + " taskParams == nullptr   received invalid parameters, deleting task counter " + String(counter) + " taskParams->taskName " + String(taskParams->taskName));
        vTaskDelete(NULL);
    }

    if (taskParams->xSemaphore == nullptr)
    {
        syslog_ng("TaskTemplate  xSemaphore == nullptr  received invalid parameters, deleting task counter " + String(counter) + " taskParams->taskName " + String(taskParams->taskName));
        vTaskDelete(NULL);
    }

    if (taskParams->taskName == nullptr)
    {
        syslog_ng("TaskTemplate  taskName == nullptr  received invalid parameters, deleting task counter " + String(counter) + " taskParams->taskName " + String(taskParams->taskName));
        vTaskDelete(NULL);
    }
    vTaskDelay(3000);
    for (;;)
    {
        ...
        if (timeSinceLastExecution >= taskParams->repeatInterval)
        {
            ...
            if (log_debug)
            {
                syslog_ng("TaskTemplate  " + String(taskParams->taskName) + " Starting vTaskDelay counter " + String(counter) + "taskName" + String(taskParams->taskName));
                syslog_ng("TaskTemplate  " + String(taskParams->taskName) + " Starting taskName" + String(taskParams->taskName));
                syslog_ng("TaskTemplate  " + String(taskParams->taskName) + " timeSinceLastExecution >= taskParams->repeatInterval Starting taskName: " + String(taskParams->taskName));
            }
            if (xSemaphoreTake(taskParams->xSemaphore, (TickType_t)1) == pdTRUE)
            {
                ...
                syslog_ng(" Start  " + String(taskParams->taskName) + " TaskTemplateCounter " + String(counter) + String(taskParams->taskName));
                ...
                syslog_ng(String(taskParams->taskName) + " End, currentStackSize " + String(currentStackSize) + " Execution time: " + String(executionTime) + "ms");
...
struct SensorData
{
    ...
    String addressToString() const
    {
        ...
        {
            if (address[i] < 16)
                str += "0"; // Добавляем ведущий ноль для однозначных значений
...
void enqueueMessage(const char *topic, const char *payload)
{
    ...
    if (mqttClient.connected())
    {
        ...
        if (packet_id == 0)
        {
            ...
            syslog_ng(" error enqueueMessage: publish packet_id " + String(packet_id) + " topic " + String(topic) + " payload " + String(payload));
        }
    }
#endif
...
float calculateSVP(float temperature)
{
    return 0.61078 * exp((17.27 * temperature) / (temperature + 237.3));
}
...
void publish_parameter(const String &key, float value, int precision, int timescale)
{
    ...
    if (timescale == 1)
    {
        ...
        char valueStr[32];
        if (log_debug)
            syslog_ng("publish_parameter topic " + topic + " value: " + String(value));
        dtostrf(value, 1, precision, valueStr); // Convert float to string with precision
        if (log_debug)
            syslog_ng("published topic " + topic + " value: " + String(value));
...
Group groups[] = {
    ...
    {"Калибровка", 21, {
                           ...
                           {"tR_DAC", "Параметр tR_DAC для NTC", Param::FLOAT, .defaultFloat = tR_DAC}, {"tR_B", "Параметр tR_B для NTC", Param::FLOAT, .defaultFloat = tR_B}, {"tR_val_korr", "Линейная коррекция NTC", Param::FLOAT, .defaultFloat = tR_val_korr}, {"tR_type", "Тип датчика температуры NTC", Param::STRING, .defaultString = tR_type.c_str()},
                           ...
                           //    {"max_l_level", "Макс. уровень жидкости", Param::FLOAT, .defaultFloat = lc.first() ? lc.first().level : 0},
                           //    {"max_l_raw", "Сырой макс. уровень жидкости", Param::FLOAT, .defaultFloat = lc.first() ? lc.first().raw : 0},
                           //    {"min_l_level", "Мин. уровень жидкости", Param::FLOAT, .defaultFloat = lc.last() ? lc.last().level : 0},
                           //    {"min_l_raw", "Сырой мин. уровень жидкости", Param::FLOAT, .defaultFloat = lc.last() ? lc.last().raw : 0},
