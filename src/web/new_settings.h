...
void handleRoot()
{
  ...
  server.send_P(200, "text/html", _Users_mmatveyev_PycharmProjects_web_calc_local_files_api_test_out_html_gz, _Users_mmatveyev_PycharmProjects_web_calc_local_files_api_test_out_html_gz_len);
}
  ...
  if (server.hasArg("plain"))
  {
    ...
    for (JsonPair kv : doc.as<JsonObject>())
    {
      ...
      if (PreferenceItem *item = findPreferenceByKey(settingName))
      {
        ...
        else if (strncmp(settingName, "DRV", 3) == 0 ||
                 strncmp(settingName, "PWD", 3) == 0 ||
