...
{
  if (isnan(my_arg) || my_arg == 0.0)
    return NAN; // Check if my_arg is not a number or zero
  ...
  {
    if (my_arg > pR_DAC)
      return NAN; // Return NaN for invalid range in reverse mode
...
float fpr(float my_arg)
{
  ...
  float ppa = -(-ppx1 * ppy3 + ppx1 * ppy2 - ppx3 * ppy2 + ppy3 * ppx2 + ppy1 * ppx3 - ppy1 * ppx2) * denominator_inv / (ppx1_sq * ppx3 - ppx1_sq * ppx2 + ppx1 * ppx2_sq - ppx1 * ppx3_sq + ppx3_sq * ppx2 - ppx3 * ppx2_sq);
  float ppb = (ppy3 * ppx2_sq - ppx2_sq * ppy1 + ppx3_sq * ppy1 + ppy2 * ppx1_sq - ppy3 * ppx1_sq - ppy2 * ppx3_sq) * denominator_inv;
  float ppc = (ppy3 * ppx1_sq * ppx2 - ppy2 * ppx1_sq * ppx3 - ppx2_sq * ppx1 * ppy3 + ppx3_sq * ppx1 * ppy2 + ppx2_sq * ppy1 * ppx3 - ppx3_sq * ppy1 * ppx2) * denominator_inv;
