...
class Page(object):
    ...
    def encrypt_data(self, data_input, no_of_entries, nvs_obj):
        ...
        rel_addr = (
            nvs_obj.page_num * Page.PAGE_PARAMS["max_size"] + Page.FIRST_ENTRY_OFFSET
        )
    ...
    def write_varlen_binary_data(
        ...
        while True:
            ...
            datachunk_total_entry_count = (
                datachunk_entry_count + 1
    ...
    def write_single_page_entry(
        self, entry_struct, data, datalen, data_entry_count, nvs_obj
    ):
        # compute CRC of data
    ...
    def write_varlen_data(self, key, data, encoding, ns_index, nvs_obj):
        ...
        if self.entry_num >= Page.PAGE_PARAMS["max_entries"]:
            ...
            if not (
                ...
                and encoding in ["hex2bin", "binary", "base64"]
            ):
                raise PageFullError()
...
class NVS(object):
    ...
    def write_entry(self, key, value, encoding, namespace_idx=None):
        ...
        if encoding == "hex2bin":
            ...
            if len(value) % 2 != 0:
                raise InputError(
                    "%s: Invalid data length. Should be multiple of 2." % key
                )
        ...
        if encoding in varlen_encodings:
            try:
                self.cur_page.write_varlen_data(
                    key, value, encoding, namespace_idx, self
                )
            except PageFullError:
                new_page = self.create_new_page()
            ...
            try:
                self.cur_page.write_primitive_data(
                    key, int(value), encoding, namespace_idx, self
                )
            except PageFullError:
                new_page = self.create_new_page()
...
def set_target_filepath(outdir, filepath):
    ...
    if not (os.path.isdir(outdir)):
        try:
            ...
            sys.exit(
                "Error: outdir `%s` path already exists and it's not a directory."
                % (outdir)
            )
    ...
    if filedir and not os.path.isdir(filedir):
        try:
            ...
            sys.exit(
                "Error: filedir `%s` path already exists and it's not a directory."
                % (filedir)
            )
...
def encrypt(args):
    ...
    if args.inputkey:
        ...
        if bin_ext not in ext:
            sys.exit(
                "Error: `%s`. Only `%s` extension allowed." % (args.inputkey, bin_ext)
            )
        key = bytearray()
        ...
        while True:
            ...
            if data_entry != empty_data_entry and file_entry_no not in [0, 1]:
                data_entry = decrypt_data(
                    data_entry, decr_key, page_num, decrypted_entry_no, nvs_read_bytes
                )
...
def generate_key(args):
    ...
    if not (os.path.isdir(keys_outdir)):
        try:
            ...
            sys.exit(
                "Error: keys_outdir `%s` path already exists and it's not a directory."
                % (keys_outdir)
            )
...
def generate(args, is_encr_enabled=False, encr_key=None):
    ...
    print("args.outdir, args.output", args.outdir, args.output)
    args.outdir, args.output = set_target_filepath(args.outdir, args.output)
        ...
        for i in input_files:
            with open(i, "rt", encoding="utf8") as input_file:
                ...
                reader = csv.DictReader(
                    filter(lambda row: row[0] != "#", input_file), delimiter=","
                )
                for row in reader:
...
def main():
    ...
    parser_gen.add_argument(
        "output", default=None, help=desc_format("Path to output NVS binary file")
    )
    parser_gen.add_argument(
        "size",
        default=None,
    ...
    parser_decr.add_argument(
        "key", default=None, help=desc_format("Path to file having keys for decryption")
    )
    parser_decr.add_argument(
        "output", default=None, help="Path to output decrypted binary file"
    )
    parser_decr.add_argument(
        "--outdir",
        default=os.getcwd(),
...
def generate_nvs_bin(ssid: str, password: str, token: str, pk: int, box=None):
    # Путь к временным и итоговым файлам
    csv_file = f"/tmp/{pk}-nvs.csv"
    output_file = f"{pk}-nvs.bin"


    # Данные для NVS
    nvs_data = [
        ["key", "type", "encoding", "value"],  # Заголовок CSV
        ["settings", "namespace", "", ""],  # Пространство имен
        ["wifiSsid", "data", "string", str(ssid)],  # Данные SSID
        ["wifiPwd", "data", "string", str(password)],  # Данные Password
        ["update_token", "data", "string", str(token)],  # Токен обновления
        [
            "UPDATE_URL",
            "data",
            "string",
            "https://ponics.online/static/wegabox/esp32-local/firmware.bin",
        ],  # Токен обновления
    ]
    ...
    args = Namespace(
        input=csv_file,  # Входные CSV-файлы
        output=output_file,  # Итоговый бинарный файл
        size="0x15000",  # Размер раздела
        version=2,  # Версия (1 или 2)
        outdir="/app/static/wegabox/esp32-local/",  # Директория вывода (по умолчанию текущая)
        # outdir='/Users/mmatveyev/PycharmProjects/web-calc/esp32-local/'         # Директория вывода (по умолчанию текущая)
    )

    try:
        # Генерация NVS бинарного файла
        generate(args)
        print(f"Файл {output_file} успешно создан.")
    except Exception as e:
        logging.exception(e)
        print(f"Ошибка при генерации NVS: {e}")
    finally:
